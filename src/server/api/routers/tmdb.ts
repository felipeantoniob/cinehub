...
import type { MovieCredits, MovieDetails, MovieListResponse, TrailerListResponse } from '~/types'
...
export const tmdbRouter = createTRPCRouter({
  getPopularMovies: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const endpoint = new URL(`${API_ORIGIN}${POPULAR_MOVIES_PATHNAME}?${params}`)
  ...
  searchMovies: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const endpoint = new URL(`${API_ORIGIN}${SEARCH_MOVIES_PATHNAME}?${params}`)
      const response = await fetch(endpoint)
  ...
  getMovieDetails: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const endpoint = new URL(`${API_ORIGIN}${movieDetailsPathname}?${params}`)
      const response = await fetch(endpoint)
  ...
  getMovieCredits: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const endpoint = new URL(`${API_ORIGIN}${movieCreditsPathname}?${params}`)
      const response = await fetch(endpoint)
  ...
  getMovieTrailers: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const endpoint = new URL(`${API_ORIGIN}${movieTrailersPathname}?${params}`)
      const response = await fetch(endpoint)
